    ...
    void OnControllerColliderHit(ControllerColliderHit hit)
{
    if (hit.gameObject.name == "Player" || hit.gameObject.CompareTag("Player"))
    {
        ...
        void Update()
        {
            ...
            {
                prefVelocity = new Vector2(playerPosition.x - position.x, playerPosition.z - position.z);

                // prefVelocity =  new Vector2(Player.transform.position.x - position.x, Player.transform.position.z - position.z);
